...
void MosekCloneParameters(MSK::Task_t tdst, MSK::Task_t tsrc) {
    auto numdpar = MSK::get_num_double_param();
    auto numipar = MSK::get_num_int_param();

    std::vector<int32_t> ipar(numipar);
    std::vector<double>  dpar(numdpar);
    MSK::get_all_double_params(tsrc,numdpar,dpar.data());
    MSK::get_all_int_params(tsrc,numipar,ipar.data());

    MSK::put_all_double_params(tdst,numdpar,dpar.data());
    MSK::put_all_int_params(tdst, numipar,ipar.data());
}
...
// function.
class MosekMPCallbackContext : public ::operations_research::MPCallbackContext {
  public:
...
MosekMPCallbackContext::MosekMPCallbackContext() :
    ev{} {
}

int64_t MosekMPCallbackContext::NumExploredNodes() {
  int infidx = MSK::get_iinf_index("iinf_mio_num_solved_nodes");
    ...
    break_solver_(false)
{
  ...
  MSK::put_obj_sense(task_, maximize_ ? MSK::ObjSense::MAXIMIZE : MSK::ObjSense::MINIMIZE);
  CheckedMosekCall(MSK::append_rows(task_,1));
  CheckedMosekCall(MSK::put_obj_row(task_,0));
...
void MosekInterface::Reset() {
  ...
  if (! new_task) {
    ...
    if (MSK::RES_OK != MSK::append_rows(task_,1) ||
        MSK::RES_OK != MSK::put_obj_row(task_,0))
      throw std::bad_alloc();
  }
...
void MosekInterface::SetOptimizationDirection(bool maximize) {
  ...
  MSK::put_obj_sense(task_, maximize ? MSK::ObjSense::MAXIMIZE : MSK::ObjSense::MINIMIZE);
}
...
void MosekInterface::SetConstraintBounds(int index, double lb, double ub) {
  ...
  if (i >= 0) {
    ...
    double rhs[2] = { std::isfinite(lb) ? lb : 0.0 , std::isfinite(ub) ? ub : 0.0 };
    ...
    double b[4] = {0.0, 1.0, std::isfinite(lb) ? lb : 0.0 , std::isfinite(ub) ? ub : 0.0 };
...
void MosekInterface::AddRowConstraint(MPConstraint* const ct) {
  ...
  int64_t numrow[2] = { 1, 1 };
  double  rhs[2]     = { std::isfinite(lb) ? lb : 0.0, std::isfinite(ub) ? ub : 0.0 };

  CheckedMosekCall(MSK::append_cons(task_,2,domidxs,2,afeidxs,rhs));
...
bool MosekInterface::AddIndicatorConstraint(MPConstraint* const ct) {
  ...
  CheckedMosekCall(MSK::append_empty_djcs(task_, 1));
  CheckedMosekCall(MSK::append_rows(task_, 2));
...
void MosekInterface::AddVariable(MPVariable* const var) {
  ...
  CheckedMosekCall(MSK::append_vars(task_, 1));
  double lb = var->lb();
  double ub = var->ub();
...
void MosekInterface::SetCoefficient(MPConstraint* const constraint,
    ...
    auto j = variable->index();
    if (i >= 0) {
        CheckedMosekCall(MSK::put_ijc(task_, afei,j, new_value));
    } else {
        int64_t djci = -i-1;
        CheckedMosekCall(MSK::put_ijc(task_, afei+1, variable->index(), new_value));
    }
}
...
void MosekInterface::ClearConstraint(MPConstraint* const constraint) {
  ...
  auto [coni,afei] = mp_cons_to_mosek_cons_[constraint->index()];
  if (coni >= 0) {
    int64_t afeidxs[2] = {afei,afei};
    int64_t domidxs[2] = {domidx_rfree,domidx_rfree};
    double rhs[2] = {0.0,0.0};
    CheckedMosekCall(MSK::put_con_slice(task_,coni,2,2,domidxs,afeidxs,rhs));
  } else {
    int64_t djci = -coni-1;
    CheckedMosekCall(
...
void MosekInterface::SetObjectiveCoefficient(const MPVariable* const variable, double coefficient) {
  InvalidateSolutionSynchronization();
...
void MosekInterface::SetObjectiveOffset(double value) {
  InvalidateSolutionSynchronization();
  CheckedMosekCall(MSK::put_row_g(task_,0,value));
}

void MosekInterface::ClearObjective() {
  InvalidateSolutionSynchronization();
  int numvar = MSK::get_num_var(task_);
  for (int i = 0; i < numvar; ++i) MSK::put_ijc(task_, 0,i, 0.0);
...
int64_t MosekInterface::iterations() const {
  ...
  int32_t index;
  if (0 <= (index = MSK::get_iinf_index("iinf_sim_dual_iter")))
    MSK::get_iinf(task_,index,&psim_iter);
  if (0 <= (index = MSK::get_iinf_index("iinf_sim_primal_iter")))
    MSK::get_iinf(task_,index,&dsim_iter);
  if (0 <= (index = MSK::get_iinf_index("iinf_intpnt_iter")))
    MSK::get_iinf(task_,index,&intpnt_iter);
  if (0 <= (index = MSK::get_liinf_index("liinf_mio_intpnt_iter")))
    MSK::get_liinf(task_,index,&mio_intpnt_iter);
  if (0 <= (index = MSK::get_liinf_index("liinf_mio_simplex_iter")))
...
int64_t MosekInterface::nodes() const {
  ...
  int nnodes = 0;
  int index = MSK::get_iinf_index("iinf_mio_num_solved_nodes");
  if (index >= 0)
      MSK::get_iinf(task_, index, &nnodes);
  return nnodes;
}
...
MPSolver::BasisStatus MosekInterface::row_status(int constraint_index) const {
  ...
  int nsol = MSK::get_num_sol(task_);
  // locate a basic solutoin
  MSK::SolType st;
  for (int soli = 0; soli < nsol; ++soli) {
    MSK::get_sol_type(task_,soli,&st);
    if (st == MSK::SolType::BASIC) {
      int lowbas,uprbas;
      int lowbind,uprbind;
      MSK::get_sol_basic_con(task_,soli,coni,&lowbas);
      MSK::get_sol_basic_con(task_,soli,coni+1,&uprbas);
      MSK::get_sol_sta_con(task_,soli,coni,&lowbind);
      MSK::get_sol_sta_con(task_,soli,coni+1,&uprbind);
...
bool MosekInterface::SetSolverSpecificParametersAsString(
  ...
  while (data.size() > 0) {
    ...
    if (eq_pos != std::string_view::npos) {
      ...
      if (MSK::RES_OK != MSK::put_param_str(task_, key.c_str(), value.c_str(),&ok)) {
...
void MosekInterface::SetRelativeMipGap(double value) {
    int ok;
  CheckedMosekCall(MSK::put_double_param(task_, "dpar_mio_rel_gap_const", value,&ok));
}

void MosekInterface::SetPrimalTolerance(double value) {
    int ok;
  CheckedMosekCall(MSK::put_double_param(task_, "dpar_intpnt_tol_pfeas", value,&ok));
  CheckedMosekCall(MSK::put_double_param(task_, "dpar_basis_tol_x", value,&ok));
}

void MosekInterface::SetDualTolerance(double value) {
    int ok;
  CheckedMosekCall(MSK::put_double_param(task_, "dpar_intpnt_tol_dfeas", value,&ok));
  CheckedMosekCall(MSK::put_double_param(task_, "dpar_basis_tol_s", value,&ok));
}
...
void MosekInterface::SetPresolveMode(int value) {
  ...
  switch (value) {
    case MPSolverParameters::PRESOLVE_OFF: {
      CheckedMosekCall(MSK::put_param_str(task_, "ipar_presolve_use", "off",&ok));
      break;
    }
    case MPSolverParameters::PRESOLVE_ON: {
      CheckedMosekCall(MSK::put_param_str(task_, "ipar_presolve_use", "on",&ok));
      break;
    }
    default: {
...
void MosekInterface::SetScalingMode(int value) {
  ...
  switch (value) {
    case MPSolverParameters::SCALING_OFF:
      CheckedMosekCall(
          MSK::put_param_str(task_, "ipar_intpnt_scaling", "scaling_none",&ok));
      CheckedMosekCall(
          MSK::put_param_str(task_, "ipar_sim_scaling", "scaling_none",&ok));
      break;
    case MPSolverParameters::SCALING_ON:
      CheckedMosekCall(
          MSK::put_param_str(task_, "ipar_intpnt_scaling", "scaling_free",&ok));
      CheckedMosekCall(
          MSK::put_param_str(task_, "ipar_sim_scaling", "scaling_free", &ok));
      break;
    default:
...
void MosekInterface::SetLpAlgorithm(int value) {
  ...
  switch (value) {
    case MPSolverParameters::DUAL:
      CheckedMosekCall(MSK::put_param_str(task_, "ipar_optimizer","optimizer_dual_simplex",&ok));
      break;
    case MPSolverParameters::PRIMAL:
      CheckedMosekCall(MSK::put_param_str(task_, "ipar_optimizer","optimizer_primal_simplex", &ok));
      break;
    case MPSolverParameters::BARRIER:
      CheckedMosekCall(
          MSK::put_param_str(task_, "ipar_optimizer", "optimizer_intpnt",&ok));
      break;
    default:
...
MPSolver::ResultStatus MosekInterface::Solve(const MPSolverParameters& param) {
  ...
  // Set log level.
  int ok;
  CheckedMosekCall(MSK::put_int_param(task_, "ipar_log", quiet_ ? 0 : 10, &ok));
  ...
  for (int j = 0; j < numvar && ! ismip; ++j) {
      MSK::VariableType vt;
      MSK::get_var_type(task_,j,&vt); ismip = ismip && vt == MSK::VariableType::INTEGER;
  }
  ...
  if (solver_->solution_hint_.size() > 0) {
    ...
    int soli = MSK::get_num_sol(task_);
    MSK::append_sol(task_,MSK::SolType::BASIC);
    MSK::put_sol_xx(task_,soli,xx.size(),xx.data());
  }

  // Time limit.
  if (solver_->time_limit() != 0) {
    VLOG(1) << "Setting time limit = " << solver_->time_limit() << " ms.";
    CheckedMosekCall(MSK::put_double_param(task_, "dpar_optimizer_max_time",solver_->time_limit_in_secs(),&ok));
  }
  ...
  // result.
  MSK::put_param_str(task_, "ipar_remove_unused_solutions", "on",&ok);
  ...
  MSK::TrmCode trm;
  {
    MosekMPCallbackContext mosek_context{};
    ...
    auto _remove_cb = absl::MakeCleanup([&]() {
        MSK::clear_stream_callback(task_,MSK::StreamType::LOG);
    });
    ...
    //                         StreamCallbackImpl);
    MSK::put_stream_callback(task_,MSK::StreamType::LOG, &mp_callback_with_context, StreamCallbackImpl);
    CheckedMosekCall(MSK::optimize_callback(task_,&trm, &mp_callback_with_context, CallbackImpl, nullptr,nullptr));
  ...
  VLOG(1) << absl::StrFormat("Solved in %s.",
                             absl::FormatDuration(timer.GetDuration()));
  // Get the status.
  const int solution_count = MSK::get_num_sol(task_);
  MSK::ProSta prosta;
  MSK::SolSta psta,dsta;
  MSK::SolType whichsol;
  int sol_index = -1;
  if (solution_count > 0) {
      sol_index = 0;
      CheckedMosekCall(MSK::get_problem_status(task_,sol_index,&prosta));
      CheckedMosekCall(MSK::get_sol_status(task_,sol_index,&psta,&dsta));
      CheckedMosekCall(MSK::get_sol_type(task_,sol_index,&whichsol));


      if (       psta == MSK::SolSta::OPTIMAL
          ||     psta == MSK::SolSta::INTEGER_OPTIMAL) {
          result_status_ = MPSolver::OPTIMAL;
      } else if (psta == MSK::SolSta::FEASIBLE && dsta == MSK::SolSta::FEASIBLE) {
          result_status_ = MPSolver::FEASIBLE;
      } else if (dsta == MSK::SolSta::INFEAS_CERT) {
        result_status_ = MPSolver::INFEASIBLE;
      } else if (psta == MSK::SolSta::INFEAS_CERT) {
  ...
  if (whichsol == MSK::SolType::INTEGER &&
      (result_status_ == MPSolver::FEASIBLE ||
       result_status_ == MPSolver::OPTIMAL)) {
    MSK::get_dinf(task_, MSK::get_dinf_index("dinf_mio_obj_bound"), &best_objective_bound_);
  ...
  if (solution_count > 0 && (result_status_ == MPSolver::FEASIBLE ||
                             result_status_ == MPSolver::OPTIMAL)) {
    // Get the results.
    MSK::get_primal_obj(task_, sol_index, &objective_value_);
    VLOG(1) << "objective = " << objective_value_;

    std::vector<double> xx(numvar);
    CheckedMosekCall(MSK::get_sol_xx_slice(task_, sol_index, 0, numvar, xx.data()));
    {
    ...
    if (whichsol != MSK::SolType::INTEGER) {
      {
        ...
        CheckedMosekCall(MSK::get_sol_slx_slice(task_,sol_index,0,numvar,slx.data()));
        CheckedMosekCall(MSK::get_sol_sux_slice(task_,sol_index,0,numvar,sux.data()));
...

      {
        ...
        std::vector<double> y(numcon*2);

        CheckedMosekCall(MSK::get_sol_y_slice(task_,sol_index,0,numcon*2,numcon*2,y.data()));

        for (int i = 0; i < solver_->constraints_.size(); ++i) {
          MPConstraint* const ct = solver_->constraints_[i];
          auto [coni,afei] = mp_cons_to_mosek_cons_[ct->index()];
          if (coni >= 0) {
            ct->set_dual_value(y[coni] - y[coni+1]);
...
void MosekInterface::Write(const std::string& filename) {
  ...
  MSK::ResCode r = MSK::write_task_to_file(task_, filename.c_str());
  if (MSK::RES_OK != r) {
...
namespace {
    const void * const kRegisterMosekLp ABSL_ATTRIBUTE_UNUSED = [] {
        MPSolverInterfaceFactoryRepository::GetInstance()->Register(
            [](MPSolver*solver) {
                return new MosekInterface(solver,false);
            },
            MPSolver::MOSEK_LINEAR_PROGRAMMING,
            []() {
                return 0 == MSK120::initialize_library();
            });
        return nullptr;
    }();
    const void * const kRegisterMosekMip ABSL_ATTRIBUTE_UNUSED = [] {
        MPSolverInterfaceFactoryRepository::GetInstance()->Register(
