...
class MosekSolver : public SolverInterface {
 ...
 private:
  using Task_t = ::operations_research::MSK120::Task_t;
  using Task_s = ::operations_research::MSK120::Task_s;
  using Task_deleter = void(*)(Task_t);

  std::tuple<const char *,const char *,std::string> last_error();

  std::unique_ptr<Task_s,Task_deleter> taskp;
  Task_t task;

  /// Maps Variable ID to MOSEK variable index
  absl::flat_hash_map<int64_t, int32_t> variable_map;
  /// Maps Constraint ID to MOSEK constraint index for lower and upper bounds and row index.
  absl::flat_hash_map<int64_t, std::tuple<int64_t,int64_t,int64_t>> linconstr_map; // triplet (acc_low, acc_upr, afe_row)
  /// Maps ConeConstraint ID to MOSEK constraint and row indexes.
  absl::flat_hash_map<int64_t, int64_t> coneconstr_map;
  absl::flat_hash_map<int64_t, int64_t> indconstr_map; // djc index:
  ...
  absl::StatusOr<PrimalSolutionProto> PrimalSolution(
      int sol_index, const std::vector<int64_t>& ordered_xx_ids,
      bool skip_zeros);
  absl::StatusOr<DualSolutionProto> DualSolution(
      int sol_index, const std::vector<int64_t>& ordered_y_ids,
      bool skip_y_zeros, const std::vector<int64_t>& ordered_yx_ids,
      bool skip_yx_zeros);
  absl::StatusOr<SolutionProto> Solution(
      int sol_index, const std::vector<int64_t>& ordered_xc_ids,
  ...
  MosekSolver(std::unique_ptr<Task_s,Task_deleter> && task);
};
